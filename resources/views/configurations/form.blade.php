...
<div class="form-group clearfix {{ $errors->has('entrydate') ? 'has-error' : '' }}">
    ...
    <div class="col-md-9 col-sm-6 float-right">
        <input class="form-control " name="entrydate" type="text" id="entrydate" value="{{date('Y-m-d'), old('entrydate', optional($configurations)->entrydate) }}" minlength="1" placeholder="Enter entrydate here..." readonly>
...
<div class="form-group clearfix {{ $errors->has('createdby') ? 'has-error' : '' }}">
    ...
    <div class="col-md-9 col-sm-6 float-right">
        <input class="form-control " name="createdby" type="text" id="createdby" value="{{Auth::user()->name, old('createdby', optional($configurations)->createdby) }}" minlength="1" placeholder="Enter createdby here..." readonly>
...
<div class="form-group clearfix {{ $errors->has('company_id') ? 'has-error' : '' }}">
    ...
    <div class="col-md-9 col-sm-6 float-right">
        <select class="form-control" id="company_id" name="company_id">
        	...
        	@foreach ($companies as $key => $companys)
			    <option value="{{ $companys }}" {{ old('company_id', optional($configurations)->company_id) == $companys ? 'selected' : '' }}>
			    	{{ $companys }}
			    </option>
			@endforeach
        </select>
