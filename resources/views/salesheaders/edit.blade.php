...
@section('page-title','salesheaders Edit')
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">salesheaders</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
			  <li class="breadcrumb-item"><a href="{{ route('salesheaders.salesheader.index') }}">salesheaders</a></li>
			...
			<div class="row">
				<div class="col-12">
					<div class="card card-primary card-outline">
						<form method="POST" action="{{ route('salesheaders.salesheader.update', $salesheader->id) }}" id="edit_salesheader_form" ng-controller="SalesHeaderInventoryController" name="edit_salesheader_form" accept-charset="UTF-8" class="form-horizontal">
							<div class="card-header">
								<h3 class="card-title clearfix">
								   {{ !empty($title) ? $title : 'salesheader' }}
								   <div class="btn-group btn-group-sm float-right"  role="group">
										<a href="{{ route('salesheaders.salesheader.index') }}" class="btn btn-primary btn-sm" title="Show All salesheaders">
											<span class="fa fa-th-list" aria-hidden="true"></span>
										</a>

										<a href="{{ route('salesheaders.salesheader.create') }}" class="btn btn-success btn-sm" title="Create New salesheaders">
...
$scope.table_products         = @json($salesheader->InvoicenoList->map(function($r){
  $r->po_id=$r->productcode_id;
  return $r;
}));
$scope.tax_type         = @json($salesheader->taxtypes);;
$scope.discount_amt     = @json($salesheader->discountamount);
$scope.Packing_amt      = @json($salesheader->Packing_amt);
$scope.Freight_amt      = @json($salesheader->Freight_amt);
$scope.other_amt        = @json($salesheader->otheramount);
    
$scope.podate           = @json($salesheader->podate);
$scope.invoiceto        = @json($salesheader->invoiceto);
$scope.customercode_id  = @json($salesheader->customercode_id);
// $scope.ponumber         = @json($salesheader->ponumber);
$scope.rupees = '';
$scope.paise = '';
$scope.number='';    
$scope.products=[];
$scope.shopcode="{{$salesheader->shopcode}}";
$scope.plantcode ="{{$salesheader->plantcode}}";
$scope.pdsno ="{{$salesheader->pdsno}}";
$scope.pdsdate ="{{$salesheader->pdsdate}}";
$scope.taxamountword        = @json($salesheader->taxamountword);
$scope.grandtotalamountword = @json($salesheader->grandtotalamountword); 
$scope.old_po_arr=@json($salesheader->InvoicenoList()->with('purchaseorder.podetail')->where('ponumber','!=',null)->groupBy('ponumber')->get()->pluck('purchaseorder')->all());
var oldinput_arr=$scope.old_po_arr.map(old_po=>old_po.pono);
$scope.products=@json($salesheader->InvoicenoList()->with('purchaseorder.podetail')->where('ponumber','!=',null)->groupBy('ponumber')->get()->pluck('purchaseorder.podetail')->flatten());
console.log([$scope.old_po_arr,oldinput_arr,$scope.table_products]);
//$scope.ponumbers        = @json($salesheader->ponumber);
//$scope.customer         = @json($salesheader->customercode_id);
...
setTimeout(function() {
$scope.getcustomers();
//$scope.customercode_id=@json($salesheader->customer);
setTimeout(function(){
  $scope.getpono('{{$salesheader->customercode_id}}');
},100);

}, 100);

$scope.ponumber=$scope.old_po_arr;
...
function clearPoNum(arr_po)
{
  var po=$scope.old_po_arr.filter(e => arr_po.indexOf(e.pono)===-1)[0];
    console.log([arr_po,po,$scope.products]);
    $scope.products=$scope.products.filter(product=>product.pono_id!=po.pono);
...
//  $http.get("{{route('salesheaders.salesheader.index')}}"+'/getpono/'+$scope.customercode_id).then(function(response) {
//         response = response.data;
//         $scope.shopcode=$scope.customercode_id.shopcode||'';
//       $scope.plantcode=$scope.customercode_id.plantcode||'';
//       $scope.pdsno=$scope.customercode_id.pdsno||'';
//       $scope.pdsdate=$scope.customercode_id.pdsdate||'';
//     $scope.ponumbers=response;
//         console.log(response);
//     });
// }
...
//     $scope.table_products=$scope.table_products.filter(product=>product.ponumber!=po.pono);
//     $scope.discount_amt=parseFloat($scope.discount_amt)-parseFloat(po.discountamount);
//      $scope.Packing_amt=parseFloat($scope.Packing_amt)-parseFloat(po.Packing_amt);
//      $scope.Freight_amt=parseFloat($scope.Freight_amt)-parseFloat(po.Freight_amt);
//      $scope.other_amt=parseFloat($scope.other_amt)-parseFloat(po.otheramount);
...
//       detail.ponumber=res.pono;
//       detail.podate=res.podate;
//       $scope.table_products.push(detail);
//      });
//      $scope.podate=res.podate;
//      $scope.discount_amt=parseFloat($scope.discount_amt)+parseFloat(res.discountamount);
//      $scope.Packing_amt=parseFloat($scope.Packing_amt)+parseFloat(res.Packing_amt);
...
// // $scope.ponumber
// // oldinput_arr.forEach(po=>{
// //   console.log(po);
// // $('#ponumber').select2('val',po);
// // })
// $scope.ponumber=$scope.old_po_arr;
//     $scope.getProductsInfo=function(p){
//         $http.get("{{route('products.products.index')}}"+'/getproductlist/'+p.productcode_id+'?customer_id='+$scope.customercode_id.customercode).then(function(response) {
//             response = response.data;
// p.productcgstrate=response.productcgstrate;
// p.productcode=response.productcode;
// p.productdescription=response.productdescription;
...
//       if(t_ponumber!=null)
//       {
//         t_index=$scope.table_products.findIndex(product=>product.ponumber==t_ponumber);
//         if(t_index==-1)
//         {
//           newinput_arr=oldinput_arr.filter(arr=>arr!=t_ponumber);
//           console.log(newinput_arr);
...
//         res = response.data;
//      console.log(res);
//      $scope.table_products=res.podetail;
//      $scope.podate=res.podate;
