                ...
                <div class="card-body" ng-controller="PrintpageController">
                    ...
                    <nav style="width: 100%;background-color: #2832AB;">
                        <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                            <a class="nav-item nav-link active demo2" id="nav-UnSingned-tab"
                                style="border: 2px solid #2832AB ;" data-toggle="tab" href="#nav-UnSingned"
                                role="tab" aria-controls="nav-UnSingned" ng-click="unsigned()"
                                aria-selected="true">UnSigned Invoices</a>
                            <a class="nav-item nav-link demo2" id="nav-ReadyForSigned-tab" ng-click="readyforsigned()"
                                style="border: 2px solid #2832AB;border-left:0px" data-toggle="tab"
                                href="#nav-ReadyForSigned" role="tab" aria-controls="nav-ReadyForSigned"
                                aria-selected="false">Invoices Ready for Signature</a>
                            <a class="nav-item nav-link demo2" id="nav-Signed-tab" ng-click="signed()"
                                style="border: 2px solid #2832AB;border-left:0px" data-toggle="tab" href="#nav-Signed"
                                role="tab" aria-controls="nav-Signed" aria-selected="false">Signed Invoices</a>
                        </div>
                    </nav>
                    ...
                    <div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
                        ...
                        <div class="tab-pane fade show active" id="nav-UnSingned" role="tabpanel"
                            aria-labelledby="nav-UnSingned-tab">
                            <form role="form" action="{{ route('digitalsign.pdf') }}" method="POST"
                                id="unsigned_form" autocomplete="off">
                                {!! csrf_field() !!}
                                <div class="col-md-12 border p-3" id="unsigned_div">
                                    <div class="row vim">
                                        <div class="col-md-3">
                                            <div
                                                class="form-group clearfix{{ $errors->has('fromdate') ? 'has-error' : '' }}">
                                                <label class="lab">From Date<b class="imp">*</b></label>
                                                <input id="fromdate" class="form-control" name="fromdate"
                                                    style="border: 0px;border-bottom: 2px solid blue;"
                                                    placeholder="From date" autocomplete="off" onchange="changedate()">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div
                                                class="form-group clearfix{{ $errors->has('todate') ? 'has-error' : '' }}">
                                                <label class="lab">To Date<b class="imp">*</b></label>
                                                <input id="todate" class="form-control" name="todate"
                                                    style="border: 0px;border-bottom: 2px solid blue;"
                                                    placeholder="To date" autocomplete="off" onchange="changedate()">
                                            </div>
                                        </div>
                                        <div class="col-md-3" style="padding-top: 30px;">
                                            <button type="button" class=" btn btn-success btn-sm pl-4 pr-4"
                                                id="signbutton" style="font-weight: bold;">Click here to Sign</button>
                                            <input name="invoicenumber_ids" id="invoicenumber_ids"
                                                style="display: none;" />
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="tab-pane fade" id="nav-ReadyForSigned" role="tabpanel"
                            aria-labelledby="nav-ReadyForSigned-tab">
                            <div class="table-responsive mt-4">
                                <table id="readyforsigned_table" class="table table-bordered " width="100%"
                                    cellspacing="0">
                                    <thead>
                                        <tr>
                                            <th>Inv.No</th>
    ...
    <div class="row vim">
        ...
        <div class="col-md-3" style="padding-top: 30px;">
            ...
            <input name="invoicenumbers" id="invoicenumbers" style="display: none;" />
                    ...
                    <form role="form" action="{{ route('print.sign.pdf') }}" method="POST" id="myForm"
                        autocomplete="off">
                        {!! csrf_field() !!}
                        <div class="col-md-12 border p-3" id="full" style="display: none;">
                            <div class="row vim">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group clearfix{{ $errors->has('fromdate') ? 'has-error' : '' }}">
                                        <label class="lab">To Date<b class="imp">*</b></label>
                                        <input id="todate" class="to"
                                            style="border: 0px;border-bottom: 2px solid blue;" name="todate"
                                            placeholder="To date" autocomplete="off" ng-change="changedate()"
                                            ng-model="todate" />
                                    </div>
                                </div>
                            ...
                            <div class="row mt-2 vim1" style="padding-bottom: 15px;">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <div class="input-group mb-3">
                                            <ui-select ng-model="$parent.invnum" theme="select2"
                                                style="min-width: 250px;" title="Choose a person"
                                                append-to-body="true" id="invnum" name="invoicenumber"
                                                ng-change="checkinvoiceto1()" required>
                                                <ui-select-match allow-clear="true"
                                                    placeholder="Select a Invoice Number...">@{{ $select.selected.invoiceno }}
                                                </ui-select-match>
                                                <ui-select-choices
                                                    repeat="invoice in invoices | propsFilter: {invoiceno: $select.search}">
                                ...
                                <div class="col-md-4">
                                    <div class=" row button">
                                        <button type="submit" class=" btn btn-success btn-sm pl-4 pr-4"
                                            id="print" disabled>Print</button>
                                        <!-- <button type="button" id="download" class=" btn btn-danger btn-sm ml-2" disabled>Download</button> -->
                                        <button type="button" class="btn btn-info text-white btn-sm ml-2 pl-4 pr-4"
                                            onclick="Reset()" ng-click="clear($event, $select)">Reset</button>
                                        <button type="button"
                                            class="btn btn-warning text-white btn-sm ml-2 pl-4 pr-4"
                    ...
                    <div class="col-md-12 border p-3" id="dropedsign" style="display: none;">
                        <div class="row vim">
                            <div class="col-md-4" style="padding-bottom: 15px;" id="single">
                                <div class=" row button">
                                    <button type="button" class=" btn btn-success btn-sm pl-4 pr-4"
                                        id="signoropen">Click here to Open Signor Tool</button>
                                    <input name="invoices" id="invoices" style="display: none;" />
...
<script>
    ...
    $(function() {
        unsigned_table.on("click", "th.select-checkbox", function() {
            ...
            if (unsigned_table.rows({
                    selected: true
                }).count() !== unsigned_table.rows().count()) {
                $("th.select-checkbox").removeClass("selected");
            } else {
                $("th.select-checkbox").addClass("selected");
            }
    ...
    $('#signbutton').click(function() {
        var dataTableRows = unsigned_table.rows({
            selected: true
        }).data().toArray().map(a => a.id);
