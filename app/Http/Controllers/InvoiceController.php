...
// open an FTP connection
    ...
    public function importExcel(Request $request)
    {
        ...
        if ($v->fails()) {
            ...
            try {
                ...
                $o_sales_headers = Session::get('o_sales_header');
                $importedCount = 0;
                if ($o_sales_headers) {
                    foreach ($o_sales_headers as $sales_header) {
                        if (empty($sales_header['invoiceno'])) {
                            continue;
                        }

                        $details = $sales_header['details'] ?? [];
                        unset($sales_header['details']);

                        if (isset($sales_header['data'])) {
                            $sales_header['data'] = json_encode($sales_header['data'], true);
                        }

                        $salesheader_model = salesheader::updateOrCreate(
                            ['invoiceno' => $sales_header['invoiceno']],
                            $sales_header
                        );

                        $salesheader_model->InvoicenoList()->delete();
                        foreach ($details as $detail) {
                            $detail['invoiceno_id'] = $salesheader_model->invoiceno;
                            if (isset($detail['data'])) {
                                $detail['data'] = json_encode($detail['data'], true);
                            }

                            $sales_detail_model = new salesdetails($detail);
                            $salesheader_model->InvoicenoList()->save($sales_detail_model);
                        }
                        $importedCount++;
                    }

                    Session::forget('sales_header');
                    Session::forget('o_sales_header');
                    Session::forget('sales_details');
                }

                return redirect()->back()->with('success_message', 'Successfully imported ' . $importedCount . ' invoices directly to the database!');
    ...
    public function importExcelstore(Request $request)
    {
        ...
        try {
            ...
            // dd($o_sales_ headers,$o_sales_details);
            ...
            foreach ($sales_headers as $key => $sales_header) {
                ...
                $sales_header['ewaybillno'] = $ewaybillno[$key];
                $sales_header['vehicle_no'] = $vehicle_no[$key];
                $sales_header['tot_qty'] = $tot_qty[$key];
